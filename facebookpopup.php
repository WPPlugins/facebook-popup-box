...
Version: 1.6.2
Author: Express Tech
Author URI: https://www.xtendify.com/en/user/3-expresstech
License: GPLv2 or later
*/

/*  Copyright 2015  Express Tech  (email : hi@expresstech.io)
...
function fb_cache_wp_remote_fopen($url, $cache_prefix) {
    $cache = get_transient('fb-popup-footer-et'.$cache_prefix);
    // return $cache;
    if($cache !== false) {
        return $cache;
    } else {
        $cache = wp_remote_fopen($url);
        set_transient('fb-popup-footer-et'.$cache_prefix, $cache, 60*60*24*7);
        return $cache;
    }
}
